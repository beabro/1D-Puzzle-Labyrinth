...
// enable & initialize timer interrupts
void run_init(void) {
  enable_interrupt();
  // prepare timer
...
int decode_color(int c) {
  ...
  */
  ...
  //  colors[i] = i;
  //}
  // default to black if c out of bounds
  if (c > sizeof(colors)/sizeof(colors[0]) || c < 0) return 0; 
  return colors[c];
}
...
// main loop, calls game_loop() from game_functions.c
int main(void) {
    print("start!");
    run_init();
